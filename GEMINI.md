...
4. **⚠️ RÈGLE DE SÉCURITÉ CRITIQUE (LEÇON APPRISE) ⚠️** : Lors de la mise à jour d'enregistrements avec des données liées (ex: un bon et ses articles), **NE JAMAIS** utiliser la méthode "Supprimer les anciennes lignes puis Insérer les nouvelles" (DELETE + INSERT) depuis le client. Si la connexion échoue entre les deux opérations, cela provoque une perte de données irréversible en production. **Vous devez OBLIGATOIREMENT utiliser une transaction atomique côté serveur**, telle qu'une procédure stockée (Supabase RPC) qui gère l'opération en un seul bloc de manière sécurisée (comme `update_voucher_safely`). Appliquez systématiquement ce principe d'atomicité pour tous les projets futurs de cet utilisateur.
